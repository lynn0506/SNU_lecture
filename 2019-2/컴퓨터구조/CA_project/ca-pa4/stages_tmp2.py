...
class Control(object):
    ...
    def gen(self, inst):
        ...
        # FD_ dependency with EX_ : when FD stage's inst is either branch or R-type
        FD_EX_depend = True if(EX.reg_rd == Pipe.FD.rs1 or EX.reg_rd == Pipe.FD.rs2) and EX.reg_rd != 0 and Pipe.EX.c_rf_wen else False

        # FD_ dependency with MW_ : when FD stage's inst is either branch or R-type
        FD_MW_depend = True if(MW.reg_rd == Pipe.FD.rs1 or MW.reg_rd == Pipe.FD.rs2) and MW.reg_rd != 0 and Pipe.MW.c_rf_wen else False

        # FD_stage should be stalled until the MW_stage write its WB_data in Register
        self.FD_stall = (FD_MW_depend or FD_EX_depend or load_use_hazard) and not EX_brjmp
        self.EX_bubble = EX_brjmp or FD_EX_depend or FD_MW_depend or load_use_hazard
...
class FD(Pipe):
    ...
    def compute(self):
        ...
        # added
        self.rs2_data = Pipe.cpu.rf.read(self.rs2)
        # Generate control signals
        if not Pipe.CTL.gen(self.inst):
            self.inst = BUBBLE

        # added
        self.op1_data   = self.pc if Pipe.CTL.op1_sel == OP1_PC else rf_rs1_data
...
class EX(Pipe):
    ...
    def compute(self):
        ...
        # Calculate the branch/jump target address using an adder (forwarded to FD)
...
class MW(Pipe):
    ...
    def compute(self):
        ...
        # memory access - either read or store
        ...
        self.alu_out = mem_data if self.c_wb_sel == WB_MEM else self.alu_out
