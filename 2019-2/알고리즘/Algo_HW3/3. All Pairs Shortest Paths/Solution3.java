...
class Solution3 { //** total Time Complexity : O(V^3) **//
	...
	public static void main(String[] args) throws Exception {
		...
		for (int test_case = 1; test_case <= 10; test_case++) {
			...
			for (int i = 0; i < E; i++) {
				...
				list.add(U[i], V[i], W[i]);
			}

			// Theta O(V^2) initialization 
			for(int i = 1; i<=N; i++) {
				for(int j = 1; j<=N; j++) {
					D[i][j] = list.get(i, j);
				}
			}
			// Theta O(V^3) floyd-warshall algorithm
			for(int k = 1; k<=N; k++) {
				for(int i = 1; i<=N; i++) {
					for(int j = 1; j<=N; j++) {
						if(D[i][k] != Integer.MAX_VALUE && D[k][j] != Integer.MAX_VALUE && i != j) {
							if(D[i][k] + D[k][j] < D[i][j]) // constant time!
								D[i][j] = D[i][k] + D[k][j];
						}
					}
				}
			...
			for(int i = 1; i<= N; i++) {
				for(int j = 1; j<= N; j++) {
					if(D[i][j] != Integer.MAX_VALUE) sum += D[i][j];
				}
			}
			/////////////////////////////////////////////////////////////////////////////////////////////
			/*
	...
	public static class List {
		...
		public List(int V) {
			this.vertex = new int[V+1][V+1];
			// initialization : theta(V^2)
			for(int i = 1; i<= V; i++) {
				for(int j = 1; j<= V; j++) {
